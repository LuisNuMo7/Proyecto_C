    ...
    while (!salida)
    {
        ...
        case '$':
        {
            ...
            if (i < infixExpression.length() && infixExpression[i] == '=')
            {
                ...
                int numericValue = std::stof(numberValue); // Convertir el string a float
                variables[variableName] = numericValue;
                evaluate_int(numericValue);
                     }
            else
    {
        // Si no hay un signo igual, buscar el valor de la variable en el mapa
        auto it = variables.find(variableName);
        if (it != variables.end())
        {
            int variableValue = it->second; // Asegurarse de que el valor es un int
            evaluate_int(variableValue);
        }
        else
        {
